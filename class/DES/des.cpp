...
static const int keyWithParities[64];// 초기 설정 64비트 키값 이값은 직접 설정하는 값이다.
...
static int KeyCompressionTable[32]=
{
...
static const int RoundKeys[16][48];
...
void combine (int spl_num, int com_num,int leftBlock[],int rightBlock,int outBlock[])
{
      for(int i = 0 ; i < spl_num ; i++ )
    {
        inBlock[i] = rightBlock[i] ;
        inBlock[i+spl_num] = leftBlock[i] ;
    }
}
