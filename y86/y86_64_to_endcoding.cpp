...
void insrtruction_to_encoding()
{
	...
	const int buffer = 10;
	fscanf_s(file,"%s", __instruction, sizeof(__instruction));
	...
	case rmmovq:
	{
		...
		for (int i = 1; i < 4; i++)
		{
			rB[i - 1] = line[i]; //  line에 (를 읽기때문에 index 는 1부터. 이게아니면 인풋값에러임
		}
	...
	case mrmovq:
	{
		...
		// 메모리를 직접적으로 읽을수도있다 input D에 (%rdx)
		uint64_t D = get_integer();
		D = little_endian(D);
...
void read_label()
{
	...
	while (!feof(file))
	{
		char line[100] = { 0, };  //한라인은 100자 이내로 생각.
		//gets_s(line, 100);
